...
Benchmark suite ships with P13; this section is the *predicted* placement based on architecture. Headline expectations:

**Matmul-shaped einsums** (`ij,jk->ik`, `bij,bjk->bik`): mojo-einsum's `max_kernels` backend lowers directly to `linalg.batched_matmul`. Same kernel as MAX, so within noise of MAX's matmul calls. Should be within ±5% of PyTorch's BMM, JAX's `dot_general`, and cuBLAS direct calls on equivalent shapes. The differentiator at this shape is *call-site overhead* — see below.
