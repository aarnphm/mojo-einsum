...
| ----------------------------- | --------------- | ----------------- | ------------------ | ------------------ | ------------ | ------------ |
| Equation grammar              | ✅              | ✅                | ✅                 | ✅                 | ✅ (C API)   | ✅ (C API)   |
| Implicit output               | ✅              | ✅                | ✅                 | ✅                 | n/a          | n/a          |
| Ellipsis broadcasting         | ✅              | ✅                | ✅                 | ✅                 | n/a          | n/a          |
| Trace / diagonal              | ✅              | ✅                | ✅                 | ✅                 | n/a          | partial      |
| Multi-operand paths           | ✅              | needs `optimize=` | auto (opt_einsum)  | auto (opt_einsum)  | single-step  | single-step  |
| `greedy`                      | ✅              | ✅                | via opt_einsum     | via opt_einsum     | n/a          | n/a          |
| `optimal` DP                  | ✅ (n ≤ 16)     | ✅ (n ≲ 10)       | via opt_einsum     | via opt_einsum     | n/a          | n/a          |
| `random-greedy`               | ✅ (fixed-trial)| ❌                | via opt_einsum     | via opt_einsum     | n/a          | n/a          |
| `branch` (`all` / `2` / `1`)  | ✅              | ❌                | via opt_einsum     | via opt_einsum     | n/a          | n/a          |
| Hypergraph paths              | ❌              | ❌                | external (cotengra)| external (cotengra)| n/a          | n/a          |
| Compile-time paths            | ✅ (alias shapes)| ❌               | ❌                 | partial (jit-trace)| partial (JIT)| ❌           |
| Per-call-site kernel cache    | ✅ (P7)         | ❌                | ❌                 | ✅ (jit cache)     | ✅ (plan)    | ❌           |
| GETT fused permute            | ⏳ (P11/P12)    | ❌                | ❌                 | ❌                 | ✅           | ✅           |
| CPU tensor-core (AMX)         | ✅ (via MAX)    | partial (Accelerate)| partial          | partial            | n/a          | partial      |
| GPU tensor-core (WGMMA)       | ✅ (via MAX)    | n/a               | ✅                 | ✅                 | ✅           | n/a          |
| Configurable accumulator      | ✅              | partial           | partial            | ✅                 | ✅           | n/a          |
| Deterministic reduction       | ✅ (P9)         | partial           | partial            | partial            | ❌           | n/a          |
...
✅ shipped, ⏳ planned, ❌ out of scope. "n/a" = comparison undefined.
