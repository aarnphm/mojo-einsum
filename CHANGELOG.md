...
- **P14** `MaxGraphBackend`: Python-side lowering spec and execution now live in `_interop_max.py`. The old `_max_graph.py` alias/module is gone; `classify_pair(...)`, `lowering_spec(...)`, and `MaxGraphBackend.execute(...)` share the executable code path. Repeated labels lower through `gather_nd`, reductions cast to the accumulator dtype, and pairwise steps lower through MAX Graph matmul.
