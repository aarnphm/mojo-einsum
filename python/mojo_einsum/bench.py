"""Benchmark CLI — `python -m mojo_einsum.bench`.

Emits per-step timing as JSON. Runs each measurement N times and reports
the median (more robust to GC pauses than mean / min).

Usage:
  python -m mojo_einsum.bench "ij,jk->ik" --shapes 256,256 256,256
  python -m mojo_einsum.bench "ij,jk,kl->il" --shapes 64,64 64,64 64,64 \\
...
def _make_operands(
    shapes: list[tuple[int, ...]], dtype: np.dtype, seed: int
) -> list[np.ndarray]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.include_path:
        result["path"] = einsum_path(
            args.equation, *shapes, optimize=args.optimize
        )
