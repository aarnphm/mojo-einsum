...
Exposed callables consumed by the Python wrapper:
  ...
  - `einsum_max_f32_cpu_ptrs(eq, payload, path) -> None`, the borrowed-pointer
    Mojo MAX backend for float32 CPU buffers.
  - `einsum_max_f64_cpu_ptrs(eq, payload, path) -> None`, the borrowed-pointer
    Mojo MAX backend for float64 CPU buffers.
...
def PyInit__native() -> PythonObject:
    try:
        ...
        module.def_function[einsum_max_f32_cpu_ptrs_py]("einsum_max_f32_cpu_ptrs")
        module.def_function[einsum_max_f64_cpu_ptrs_py]("einsum_max_f64_cpu_ptrs")
...
def einsum_max_f32_cpu_ptrs_py(
    eq_obj: PythonObject,
    payload_obj: PythonObject,
    path_obj: PythonObject,
) raises -> PythonObject:
    return execute_max_ptr_payload[dtype=DType.float32, target="cpu"](
        eq_obj,
        payload_obj,
        path_obj,
    )
...
def einsum_max_f64_cpu_ptrs_py(
    eq_obj: PythonObject,
    payload_obj: PythonObject,
    path_obj: PythonObject,
) raises -> PythonObject:
    return execute_max_ptr_payload[dtype=DType.float64, target="cpu"](
        eq_obj,
        payload_obj,
        path_obj,
    )
