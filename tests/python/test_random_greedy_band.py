"""Plan §2 — `random-greedy-128` within 5% of opt_einsum optimal on n ≤ 30.

The plan's verification §2 names three targets for path quality vs
opt_einsum:
...
def _chain_shapes(n: int, dim_lo: int = 2, dim_hi: int = 16) -> tuple[str, list[tuple[int, int]]]:
  ...
  # Naive (A·B)·C·... pays ~n × dim_hi², the Bellman-optimal path pays
  # ~n × dim_hi × dim_lo.
